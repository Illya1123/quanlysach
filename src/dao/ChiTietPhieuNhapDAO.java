...
public class ChiTietPhieuNhapDAO implements DAOInterface<ChiTietPhieu> {
    ...
    public int update(ChiTietPhieu t) {
        ...
        try {
            Connection con = JDBCUtils.getConnection();
            String sql = "UPDATE ChiTietPhieuNhap SET maPhieu=?, id=?, soLuong=?, donGia = ?  WHERE maPhieu=? AND maMay=?";
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getMaPhieu());
            pst.setString(2, t.getId());
            pst.setInt(3, t.getSoLuong());
    ...
    public ArrayList<ChiTietPhieu> selectAll(String t) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                ChiTietPhieu ctp = new ChiTietPhieu(maPhieu, maSach, soLuong, donGia);
                ketQua.add(ctp);
            }
            JDBCUtils.closeConnection(con);
        } catch (Exception e) {
