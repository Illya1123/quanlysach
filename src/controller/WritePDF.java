    ...
    public void writePhieuNhap(String mapn) {
        ...
        try {
            ...
            for (ChiTietPhieu ctpn : ChiTietPhieuNhapDAO.getInstance().selectAll(mapn)) {
                Sach sach = SachDAO.getInstance().selectById(ctpn.getId());
                pdfTable.addCell(new PdfPCell(new Phrase(ctpn.getId(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(sach.getName(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(sach.getPrice()) + "đ", fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(String.valueOf(ctpn.getSoLuong()), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(ctpn.getSoLuong() * sach.getPrice()) + "đ", fontData)));
            }
    ...
    public void writePhieuXuat(String mapn) {
        ...
        try {
            ...
            for (ChiTietPhieu ctpn : ChiTietPhieuXuatDAO.getInstance().selectAll(mapn)) {
                Sach mt = SachDAO.getInstance().selectById(ctpn.getId());
                pdfTable.addCell(new PdfPCell(new Phrase(ctpn.getId(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(mt.getName(), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(mt.getPrice()) + "đ", fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(String.valueOf(ctpn.getSoLuong()), fontData)));
                pdfTable.addCell(new PdfPCell(new Phrase(formatter.format(ctpn.getSoLuong() * mt.getPrice()) + "đ", fontData)));
            }
            document.add(pdfTable);
            document.add(Chunk.NEWLINE);
            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Tổng thanh toán: " + formatter.format(pn.getTongTien()) + "đ", fontData));
