...
public class UpdatePhieuNhap extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jPanel2.add(deleteProduct, new org.netbeans.lib.awtextra.AbsoluteConstraints(100, 520, 190, 40));
    ...
    private void btnNhapHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhapHangActionPerformed
        ...
        try {
            ...
            for (var ct : CTPhieuOld) {
                ChiTietPhieuNhapDAO.getInstance().delete(ct);
            }
            // Thêm các chi tiết phiếu mới
            for (var i : CTPhieu) {
    ...
    if (i_row == -1) {
        ...
        if (mtl != null) {
            ...
            ArrayList<Sach> sachList = SearchProduct.getInstance().searchMaSach((String) tblSanPham.getValueAt(i_row, 0));
            if (!sachList.isEmpty()) { // Kiểm tra danh sách không rỗng
                Sach mt = sachList.get(0); // Lấy đối tượng Sach đầu tiên từ danh sách
                ChiTietPhieu ctp = new ChiTietPhieu(phieunhap.getMaPhieu(), String.valueOf(mt.getId()), soluong, mt.getPrice());
                CTPhieu.add(ctp);
            } else {
